...
<div class="beranda">
    ...
    <?php if ($agendas !== []): ?>
        <section class="beranda-agenda-table-card">
            <div class="beranda-table-wrap">
                <table class="beranda-agenda-table">
                    <thead>
                        <tr>
                            <th>Tanggal &amp; Waktu</th>
                            <th>Judul Agenda</th>
                    ...
                    <?php foreach ($agendas as $agenda): ?>
                        <?php
                        $sedangBerlangsung = $agenda->statusSaatIni === Agenda::STATUS_BERLANGSUNG;
                        ?>
                        <tr>
                            <td>
                                <strong><?= Html::encode(date('d M Y', strtotime($agenda->tanggal))) ?></strong>
                                <small><?= Html::encode(substr($agenda->waktu_mulai, 0, 5)) ?> - <?= Html::encode(substr($agenda->waktu_selesai, 0, 5)) ?> WIB</small>
                            </td>
                            <td>
                                <strong><?= Html::encode($agenda->pembahasan) ?></strong>
                                <small><?= Html::encode($agenda->createdBy->nama ?? 'Universitas Andalas') ?></small>
                            </td>
                            <td>
                                <strong><?= Html::encode($agenda->lokasi->lokasi ?? '-') ?></strong>
                                <small><?= Html::encode($agenda->lokasi->unit->nama_unit ?? '-') ?></small>
                            </td>
                            <td>
...
.beranda-agenda-table-card {
    margin-bottom: 20px; overflow: hidden; background: #fff;
    border: 1px solid #dfe3e8; border-radius: 14px;
}

.beranda-table-wrap { overflow-x: auto; }

.beranda-agenda-table {
    width: 100%; min-width: 900px; border-collapse: collapse;
    color: #111827; font-size: 0.84rem;
}

.beranda-agenda-table th {
    padding: 12px 14px; background: #f8f9fa; border-bottom: 1px solid #e5e7eb;
    color: #737981; font-size: 0.7rem; font-weight: 700; text-align: left;
    text-transform: uppercase; letter-spacing: 0.03em; white-space: nowrap;
}

.beranda-agenda-table td {
    padding: 14px; border-bottom: 1px solid #edf0f2; vertical-align: middle;
}

.beranda-agenda-table tbody tr:last-child td { border-bottom: 0; }
.beranda-agenda-table tbody tr:hover { background: #fbfdfb; }

.beranda-agenda-table td strong,
.beranda-agenda-table td small { display: block; }

.beranda-agenda-table td strong { margin-bottom: 4px; }
.beranda-agenda-table td strong { font-weight: 600; }
.beranda-agenda-table td small { color: #737981; font-size: 0.75rem; }

.beranda-agenda-table .beranda-badge { margin: 0; white-space: nowrap; }
...
@media (max-width: 860px) {
    ...
    .beranda-agenda-table { min-width: 820px; }
}
</style>